...
public class HLSDownloaderPlugin extends Plugin {
    ...
    private Map<String, DownloadState> downloads = new HashMap<>();
    private List<String> capturedStreams = new ArrayList<>();
    
    private static class DownloadState {
        String id, filename, url, quality, status, filePath, error;
        String estimatedQuality;  // NEW: Quality estimated from bitrate
        double bitrateMbps;       // NEW: Bitrate in Mbps
        int progress;
    ...
    private synchronized void storeCapturedStream(String url, String source) {
        ...
        try {
            ...
            notifyListeners("stream-captured", payload);
    ...
    public void getQualityVariants(PluginCall call) {
        ...
        new Thread(() -> {
            try {
                ...
                // If no variants found, return URL as default
                if (variants.length() == 0) {
                    JSObject variant = new JSObject();
                    variant.put("url", url);
                    variant.put("bandwidth", 0);
                    variant.put("label", "Default Quality");
                    variants.put(variant);
                }
                
                JSObject result = new JSObject();
                result.put("variants", variants);
                Log.d(TAG, "Returning " + variants.length() + " quality variants");
                call.resolve(result);
            } catch (Exception e) {
                Log.e(TAG, "Failed to get variants: " + e.getMessage());
                ...
                try {
                    ...
                    call.reject("Failed to get variants: " + e.getMessage());
                }
            }
        }).start();
    }
    ...
    private JSONArray parseM3U8Variants(String m3u8Url) throws Exception {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            // Look for EXT-X-STREAM-INF (variant playlist entries)
            if (line.startsWith("#EXT-X-STREAM-INF:")) {
                ...
                if (i + 1 < lines.length) {
                    ...
                    if (!variantUrl.isEmpty() && !variantUrl.startsWith("#")) {
                        ...
                        // Create label from resolution or bandwidth
                        String label;
                        if (resolution != null) {
                        ...
                        variants.put(variant);
                        Log.d(TAG, "Found variant: " + label + " (" + bandwidth + " bps)");
                    }
                }
            }
        }
    ...
    /**
     * Save downloads list to SharedPreferences for persistence
     */
    private void saveDownloadsToPreferences() {
        try {
            ...
            for (DownloadState state : downloads.values()) {
                ...
                obj.put("estimatedQuality", state.estimatedQuality);  // NEW
                obj.put("bitrateMbps", state.bitrateMbps);            // NEW
    ...
    /**
     * Load downloads list from SharedPreferences on init
     */
    private void loadDownloadsFromPreferences() {
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                state.estimatedQuality = obj.optString("estimatedQuality");  // NEW
                state.bitrateMbps = obj.optDouble("bitrateMbps", 0);         // NEW
    ...
    // NEW METHOD: Get captured streams
    @PluginMethod
    public void getCapturedStreams(PluginCall call) {
        try {
            JSONArray streams = new JSONArray();
            
            // First, try to get from stored list
            ...
            // If we have streams, return them
            if (streams.length() > 0) {
                Log.d(TAG, "Returning " + streams.length() + " captured streams");
                JSObject result = new JSObject();
            ...
            // If no streams captured, return empty array (user will need to provide URL)
            Log.d(TAG, "No captured streams available");
            JSObject result = new JSObject();
            result.put("streams", new JSONArray());
            call.resolve(result);
            
        } catch (Exception e) {
            Log.e(TAG, "Error getting captured streams", e);
            call.reject("Error: " + e.getMessage());
        }
    }

    // NEW METHOD: Capture a stream URL (called from JS/WebView)
    @PluginMethod
    ...
    // NEW METHOD: Clear captured streams
    @PluginMethod
    ...
    public void deleteDownloadedFile(PluginCall call) {
        ...
        try {
            ...
            if (file.exists()) {
                ...
                // Also remove from downloads list if it exists
        ...
        for (DownloadState state : downloads.values()) {
            try {
                array.put(new JSObject()
                    ...
                    .put("estimatedQuality", state.estimatedQuality)  // NEW
                    .put("bitrateMbps", state.bitrateMbps));          // NEW
            } catch (Exception e) {}
        }
        return array;
    }
