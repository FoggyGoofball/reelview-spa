import { getMediaDetails, getPopularMovies as tmdbGetPopularMovies, getPopularTvShows as tmdbGetPopularTvShows, getTmdbRating, tmdbFetch } from './tmdb';
import { getFullAnimeDetails, searchAnime } from '@/lib/jikan';
import { EXPLICIT_RATINGS } from './data';
// Simple in-memory cache for anime ratings to avoid hitting rate limits on searches.
const animeRatingCache = new Map();
export function tmdbMediaToBasicVideo(media) {
    var _a;
    const isMovie = media.media_type === 'movie';
    const isAsianAnimation = ['ja', 'ko', 'zh'].includes(media.original_language || '');
    let isAnimeShow = media.media_type === 'tv' && (((_a = media.genre_ids) === null || _a === void 0 ? void 0 : _a.includes(16)) && isAsianAnimation);
...
export async function tmdbMediaToVideo(media) {
    ...
    let isAnimeShow = typeToFetch === 'tv' && (((_a = detailedMedia.genres) === null || _a === void 0 ? void 0 : _a.some(g => g.id === 16)) && isAsianAnimation);
    ...
    if (isAnimeShow) {
        if (animeRatingCache.has(title)) {
            ...
            const malId = (_b = detailedMedia.external_ids) === null || _b === void 0 ? void 0 : _b.myanimelist_id;
            if (malId) {
                try {
                    const jikanDetails = await getFullAnimeDetails(String(malId));
                    if (jikanDetails === null || jikanDetails === void 0 ? void 0 : jikanDetails.rating) {
    ...
    const runtime = isMovie
        ...
        : (_c = detailedMedia.episode_run_time) === null || _c === void 0 ? void 0 : _c[0];
    const keywordsList = ((_d = detailedMedia.keywords) === null || _d === void 0 ? void 0 : _d.results) || ((_e = detailedMedia.keywords) === null || _e === void 0 ? void 0 : _e.keywords) || [];
    const video = {
        id: String(detailedMedia.id),
        title: title,
        name: title,
        description: detailedMedia.overview,
        categories: ((_f = detailedMedia.genres) === null || _f === void 0 ? void 0 : _f.map(g => g.name)) || [],
        ...
        genre_ids: ((_g = detailedMedia.genres) === null || _g === void 0 ? void 0 : _g.map(g => g.id)) || [],
        keywords: keywordsList,
        rating: rating,
        runtime: runtime,
        external_ids: detailedMedia.external_ids,
    };
    if ((_h = detailedMedia.external_ids) === null || _h === void 0 ? void 0 : _h.myanimelist_id) {
        video.mal_id = detailedMedia.external_ids.myanimelist_id;
    }
    // Final explicit check after all enrichment attempts
    const isPornographic = rating ? ['Rx', 'Hentai'].some(pr => rating.includes(pr)) : false;
    const isHorrorGenre = (_j = video.genre_ids) === null || _j === void 0 ? void 0 : _j.includes(27);
...
export async function getVideoById(id, media_type) {
    const typeToFetch = (media_type === 'anime') ? 'tv' : media_type;
    try {
...
export async function getVideosByGenre(genreId, mediaType, isKeyword = false, page = 1) {
    const typeToFetch = mediaType === 'anime' ? 'tv' : mediaType;
    const path = `/discover/${typeToFetch}`;
    let params = { sort_by: 'popularity.desc', page: String(page) };
    if (mediaType === 'anime') {
        ...
        if (isKeyword) {
            ...
            params['with_genres'] = animationGenre; // Always combine with Animation for keyword-based anime search
        }
        else {
            params['with_genres'] = `${animationGenre},${genreId}`;
        }
    }
    else if (mediaType === 'tv') {
        // For TV, fetch the genre but exclude languages that are typically anime
...
export async function searchVideos(query, isAnimeSearch = false) {
    if (!query) {
        return [];
    }
    if (isAnimeSearch) {
    ...
    const allResults = pages.filter(Boolean).flatMap(page => page.results || []);
