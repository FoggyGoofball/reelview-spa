...
export async function tmdbMediaToVideo(media: TMDBMedia): Promise<Video | null> {
  const isMovie = media.media_type === 'movie';
  const typeToFetch = ((media as any).media_type === 'anime') ? 'tv' : media.media_type;
...
export async function getVideosByGenre(genreId: string, mediaType: 'movie' | 'tv' | 'anime', isKeyword: boolean = false, page: number = 1): Promise<TMDBMedia[]> {
