...
export function saveWatchProgress(progress) {
    ...
    try {
        ...
        const key = progress.type === 'anime' && progress.mal_id
            ? `mal-${progress.mal_id}`
            : `tmdb-${progress.id}`;
        const updatedHistory = Object.assign(Object.assign({}, currentHistory), { [key]: Object.assign(Object.assign(Object.assign({}, currentHistory[key]), progress), { last_updated: Date.now() }) });
...
export function removeFromHistory(id, type, mal_id) {
    if (isServer())
        return;
    const currentHistory = getWatchHistory();
...
export function removeFromWatchlist(videoId, mediaType) {
    const currentWatchlist = getWatchlist();
    const key = `${mediaType}-${videoId}`;
    delete currentWatchlist[key];
    saveWatchlist(currentWatchlist);
}
export function isInWatchlist(videoId, mediaType) {
    const watchlist = getWatchlist();
    const key = `${mediaType}-${videoId}`;
    return !!watchlist[key];
}
// --- Custom Video Data ---
export function getCustomVideoData() {
    if (isServer())
        return {};
    try {
...
export function saveCustomVideoData(videoKey, data) {
    if (isServer())
        return;
    try {
        const currentData = getCustomVideoData();
        const updatedData = Object.assign(Object.assign({}, currentData), { [videoKey]: data });
 ...
 */
export async function enrichVideoDetails(item) {
    const isVideoObject = 'media_type' in item;
    const mediaType = isVideoObject ? item.media_type : item.type;
    const itemId = String(item.id);
    const needsEnrichment = !item.rating || !item.runtime || (mediaType !== 'movie' && !item.seasons);
    if (!needsEnrichment) {
        return item;
    }
    try {
        ...
        const finalItem = Object.assign(Object.assign({}, item), { rating: enrichedVideo.rating, runtime: enrichedVideo.runtime, is_explicit: enrichedVideo.is_explicit, seasons: enrichedVideo.seasons, episodes: enrichedVideo.episodes });
