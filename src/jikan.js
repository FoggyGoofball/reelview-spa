...
const jikanFetch = throttle(async (url, options) => {
...
export function jikanAnimeToVideo(anime, tmdbId) {
    return {
...
export async function getFullAnimeDetails(id) {
...
export async function findTmdbIdByMalId(malId) {
    ...
    try {
        ...
        const data = result.data;
        const tmdbEntry = data.find((entry) => entry.name === 'TheMovieDB');
