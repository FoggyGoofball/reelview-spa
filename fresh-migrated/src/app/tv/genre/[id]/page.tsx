  ...
  const fetchGenreVideos = async (pageNum: number, append: boolean = false) => {
    ...
    try {
      ...
      while (itemsNeeded > 0 && apiPage <= MAX_PAGES) {
        const rawMedia = await getVideosByGenre(isAdultAnimation ? '16' : genreId, 'tv', false, apiPage);
        ...
        for (const basicVideo of rawMedia) {
          ...
          if (enrichedVideo && enrichedVideo.media_type !== 'anime') {
            ...
            if (isAdultAnimation) {
              ...
              // For all other genres, add any non-anime show
              processedVideos.push(enrichedVideo);
              itemsNeeded--;
            }
          }
      ...
      console.error(`Failed to fetch videos for genre ${genreName}:`, error);
